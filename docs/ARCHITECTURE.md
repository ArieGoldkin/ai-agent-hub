...
AI Agent Hub follows a **source repository → installed configuration** architecture where the NPM package contains source materials that are transformed during installation.
...
├── instructions/        # Copied from /assets/instructions/
├── skills/              # Copied from /skills/ (v3.5.9+)
├── context/             # Freshly initialized
│   └── shared-context.json
├── commands/            # Squad mode only (from /.squad/commands/)
├── examples/            # Squad mode only (from /.squad/examples/)
├── context-triggers.md  # Copied from /assets/
└── settings.local.json  # Generated MCP configuration
...
4. Copies agents from `/agents/` → `.claude/agents/`
5. Copies instructions from `/assets/instructions/` → `.claude/instructions/`
6. Copies skills from `/skills/` → `.claude/skills/` (v3.5.9+)
...
├── assets/               # Source instruction templates
├── skills/               # Source skill modules
├── .squad/               # Squad mode source files
├── package.json
├── README.md
└── LICENSE
```
